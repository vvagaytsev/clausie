 ...
 */
final class ClauseDetector {

    /**
     * Set of dependency relations that do not belong to a complement.
     */
    private static final Set<GrammaticalRelation> EXCLUDE_RELATIONS_COMPLEMENT;

    static {
        Set<GrammaticalRelation> temp = new HashSet<>();
     ...
     */
    private static final Set<GrammaticalRelation> INCLUDE_RELATIONS_VERB;
    ...
    private static void excludeVertexes(Clause clause) {
        for (int j = 0; j < clause.constituents.size(); j++) {
            ...
            for (int k = 0; k < clause.constituents.size(); k++) {
                if (k == j || !(clause.constituents.get(k) instanceof IndexedConstituent)) continue;
     ...
     */
    private static void addNsubjClause(ClausIE clausIE,
                                       List<IndexedWord> roots,
                                       List<Clause> clauses,
                                       IndexedWord subject,
                                       IndexedWord clauseRoot,
                                       boolean partmod) {
        ...
        List<IndexedWord> ccs = ProcessConjunctions.getIndexedWordsConj(
                semanticGraph,
                clausIE.depTree,
                clauseRoot,
        ...
        for (IndexedWord root : ccs) {
            ...
            clause.verb = -1;
            SemanticGraphEdge cop = DpUtils.findFirstOfRelation(
                    outgoingEdges, EnglishGrammaticalRelations.COPULA
            );
            Set<IndexedWord> exclude;
            ...
            // relative clause?
            SemanticGraphEdge rcmod = DpUtils.findFirstOfRelation(
                    incomingEdges, EnglishGrammaticalRelations.RELATIVE_CLAUSE_MODIFIER
            );
            SemanticGraphEdge poss = null;
            if (rcmod != null)
                poss = DpUtils.findDescendantRelativeRelation(
                        semanticGraph, root, EnglishGrammaticalRelations.POSSESSION_MODIFIER
                );
...
//------------------------Set verb or complement, and subject.-------------------------------------------------
            Constituent constRoot;
            if (cop != null) {
                clause.complement = clause.constituents.size();
                constRoot = new IndexedConstituent(
                        semanticGraph,
                        root,
                ...
                if (!partmod) {
                    clause.constituents.add(
                            new IndexedConstituent(
                                    semanticGraph,
                                    cop.getDependent(),
                                    include,
                                    Collections.<IndexedWord>emptySet(),
            ...
            if (partmod) {
                ...
                List<SemanticGraphEdge> outsub = clausIE.semanticGraph.getOutEdgesSorted(subject);
                SemanticGraphEdge coppm = DpUtils.findFirstOfRelationOrDescendent(
                        outsub, EnglishGrammaticalRelations.COPULA
                );
                if (coppm != null) {
                    ((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes
                            .add(coppm.getDependent());
                    SemanticGraphEdge spm = DpUtils.findFirstOfRelationOrDescendent(
                            outsub, EnglishGrammaticalRelations.SUBJECT
                    );
                    ((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes
            ...
            for (SemanticGraphEdge outgoingEdge : outgoingEdges) {
                ...
                if (DpUtils.isComplm(outgoingEdge) || DpUtils.isMark(outgoingEdge)) {
                    ...
                    List<IndexedWord> xcomproots = new ArrayList<>();
                    List<Clause> xcompclauses = new ArrayList<>();
                    ...
                    addNsubjClause(
                            clausIE, xcomproots, xcompclauses, subject, outgoingEdge.getDependent(), false
                    );
...
//------------------------------------------------------------------------------------------------------------------          
     ...
     */
    private static void processRel(SemanticGraphEdge outgoingEdge,
                                   SemanticGraph semanticGraph,
                                   IndexedWord dependent,
                                   SemanticGraphEdge rcmod,
                                   Clause clause) {
        ...
        SemanticGraphEdge pobed = DpUtils.findFirstOfRelation(
                outdep, EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT
        );
        ...
        if (pobed != null && pobed.getDependent().tag().charAt(0) != 'W') {
            ...
            posspobj = DpUtils.findFirstOfRelation(
                    outpobj, EnglishGrammaticalRelations.POSSESSION_MODIFIER
            );
        }

        if (pobed != null && pobed.getDependent().tag().charAt(0) == 'W' && rcmod != null) {
            newSemanticGraph.addEdge(
                    dependent,
                    rcmod.getGovernor(),
    ...
    private static Constituent searchCandidateAdverbial(Clause clause) {
        for (Constituent c : clause.constituents) {
            ...
            if ("IN".equals(root.tag()) && !((IndexedConstituent) c).getSemanticGraph().hasChildren(root)) {
                return c;
            }
        }
        return null;
    ...
    private static Constituent createPossConstituent(SemanticGraph semanticGraph,
                                                     SemanticGraphEdge poss,
                                                     SemanticGraphEdge rcmod,
                                                     IndexedWord constGovernor,
                                                     Type type) {
        ...
        Set<IndexedWord> exclude = DpUtils.exclude(
                newSemanticGraph, EXCLUDE_RELATIONS_COMPLEMENT, rcmod.getGovernor()
        );
        newSemanticGraph.removeEdge(poss);
        newSemanticGraph.removeEdge(rcmod);
        return new IndexedConstituent(
                newSemanticGraph,
                constGovernor,
                Collections.<IndexedWord>emptySet(),
                exclude,
    ...
    private static IndexedConstituent createRelConstituent(SemanticGraph semanticGraph,
        ...
        SemanticGraphEdge rccop = DpUtils.findFirstOfRelation(
                outrcmod, EnglishGrammaticalRelations.COPULA
        );
        if (rccop != null) {
            Set<IndexedWord> excludercmod = DpUtils.exclude(
                    semanticGraph, EXCLUDE_RELATIONS_COMPLEMENT, root
            );
            return new IndexedConstituent(
                    semanticGraph,
                    root,
     ...
     */
    private static void addApposClause(ClausIE clausIE,
                                       IndexedWord subject,
                                       IndexedWord object) {
        Clause clause = new Clause();
        clause.subject = 0;
        ...
        clause.complement = 2;
        clause.constituents.add(
                new IndexedConstituent(clausIE.semanticGraph, subject, Constituent.Type.SUBJECT)
        );
        clause.constituents.add(
                new TextConstituent(clausIE.options.appositionVerb, Constituent.Type.VERB)
        );
        clause.constituents.add(
                new IndexedConstituent(clausIE.semanticGraph, object, Constituent.Type.COMPLEMENT)
        );
     ...
     */
    private static void addPossessiveClause(ClausIE clausIE,
                                            IndexedWord subject,
                                            IndexedWord object) {
        Clause clause = new Clause();
        ...
        Set<IndexedWord> excludesub = new TreeSet<>();
        Set<IndexedWord> excludeobj = new TreeSet<>();
        ...
        if (subject.tag().charAt(0) == 'W') {
            ...
            rcmod = DpUtils.findFirstOfRelation(
                    inedges,
            ...
            SemanticGraphEdge ps = DpUtils.findFirstOfRelation(
                    outedges,
        ...
        clause.constituents.add(
                new TextConstituent(
                        clausIE.options.possessiveVerb,
     ...
     *
     * @param subject The subject of the clause
     * @param clausIE The object of the clause
     * @param roots   List of clause roots
     */
    private static void addPartmodClause(ClausIE clausIE,
                                         IndexedWord subject,
                                         IndexedWord verb,
                                         List<IndexedWord> roots) {
     ...
     * @param roots   List of clause roots
     */
    private static void addParataxisClause(ClausIE clausIE,
                                           IndexedWord root,
                                           IndexedWord parroot,
                                           List<IndexedWord> roots) {
        ...
        SemanticGraphEdge subject = DpUtils.findFirstOfRelationOrDescendent(
                outedges,
                EnglishGrammaticalRelations.SUBJECT
        );
        if (subject != null) {
            Constituent subjectConst = new IndexedConstituent(
                    clausIE.semanticGraph,
                    subject.getDependent(),
                    Type.SUBJECT
            );
