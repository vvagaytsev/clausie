 ...
 */
public final class ProcessConjunctions {

    private ProcessConjunctions() {
    ...
    private static List<Constituent> generateConstituents(Tree depTree,
                                                          Clause clause,
                                                          IndexedConstituent constituent,
                                                          int index) {
     ...
     */
    private static void generateConstituents(SemanticGraph semanticGraph,
                                             Tree depTree,
                                             IndexedConstituent constituent,
                                             IndexedWord root,
        ...
        if (conjunct != null) {
            ...
            if (con != null && "between".equals(con.getDependent().lemma()))
                processCC = false;
            ...
            if (pobj != null && "between".equals(pobj.getGovernor().lemma()))
                processCC = false;
        ...
        for (SemanticGraphEdge edge : outedges) {
            ...
            if (DpUtils.isParataxis(edge) || DpUtils.isRcmod(edge) || DpUtils.isAppos(edge) || (DpUtils.isDep(edge) && constituent.type.equals(Type.VERB)))
            ...
            if (DpUtils.isAnyConj(edge) && processCC) {
                ...
                for (SemanticGraphEdge c : conjunct) {
                    if ("&".equals(c.getDependent().lemma()) && nextToVerb(depTree, root.index(), edge.getDependent().index(), c.getDependent().index())) {
                        cont = true;
                        break;
                    }
                }
                ...
                if (predet != null && "both".equals(predet.getDependent().lemma()))
                ...
                for (SemanticGraphEdge ed : outedges) {
                    ...
                    if (DpUtils.isPredet(ed) && "both".equals(ed.getDependent().lemma())) { //if it is one level down
                        ...
                        newSemanticGraph.addEdge(newRoot, child, reln, weight, ed.isExtra());
                    }
                }
                ...
                if (firstLevel) {
                    generateConstituents(
                            newSemanticGraph, depTree, newConstituent, newRoot, constituents, false
                    );
                } else {
                    generateConstituents(
                            newSemanticGraph, depTree, constituent, newRoot, constituents, false
                    );
                }
...
//            } else if ((DpUtils.isCc(edge) || DpUtils.isPreconj(edge))&& processCC && !edge.getDependent().lemma().equals("&")) {
            } else if ((DpUtils.isCc(edge) || DpUtils.isPreconj(edge)) && processCC && !"&".equals(edge.getDependent().value())) {
                semanticGraph.removeEdge(edge);
            } else if (!DpUtils.isPredet(edge) && !constituent.excludedVertexes.contains(edge.getDependent()))
                generateConstituents(
                        semanticGraph, depTree, constituent, edge.getDependent(), constituents, false
                );
        }
    }
    ...
    private static boolean isDescendant(Tree parse,
                                        int indexCheck,
                                        int indexPivot,
                                        int indexElement) {
        ...
        while ((!"ROOT".equals(element.value()))) {// find a common parent between the head conjoint
     ...
     */
    public static List<IndexedWord> getIndexedWordsConj(SemanticGraph semanticGraph,
                                                        Tree depTree,
                                                        IndexedWord root,
                                                        GrammaticalRelation rel,
                                                        List<SemanticGraphEdge> toRemove,
                                                        Options option) {
        List<IndexedWord> ccs = new ArrayList<>(); // to store the conjoints
        ccs.add(root);
            ...
            if (edge.getRelation().equals(rel)) {
                ...
                if (((ccVerbs || ccCop) && !notProcess) || ccMainClauses) {
                    ...
                    if (option.processCcAllVerbs || !notProcess) {
                        List<SemanticGraphEdge> conjunct = DpUtils.getEdges(
                                outedges, EnglishGrammaticalRelations.COORDINATION
                        );
     ...
     */
    private static void rewriteGraph(SemanticGraph semanticGraph,
                                     Tree depTree,
        ...
        for (int i = 0; i < ccs.size(); i++) {
            for (int j = i + 1; j < ccs.size(); j++) {
                ...
                for (SemanticGraphEdge ed : semanticGraph.getIncomingEdgesSorted(ccs.get(i))) {
                    ...
                    semanticGraph.addEdge(ed.getGovernor(), ccs.get(j), ed.getRelation(), ed.getWeight(), ed.isExtra());
                }
                ...
                for (SemanticGraphEdge ed : semanticGraph.getOutEdgesSorted(ccs.get(i))) {
                    ...
                    if (!DpUtils.isAnyConj(ed) && !DpUtils.isCc(ed)
                        ...
                        semanticGraph.addEdge(ccs.get(j), child, ed.getRelation(), ed.getWeight(), ed.isExtra());
                    }
                }
     ...
     */
    private static boolean nextToVerb(Tree depTree,
                                      int firstVerb,
                                      int secondVerb,
                                      int conj) {
     ...
     */
    private static boolean shareAll(List<SemanticGraphEdge> outedges,
                                    Tree depTree,
                                    IndexedWord root,
                                    IndexedWord conj) {
        for (SemanticGraphEdge edge : outedges) {
            if (DpUtils.isAnySubj(edge) || edge.getDependent().equals(conj))
                continue;
            else if (!isDescendant(depTree, conj.index(), root.index(), edge.getDependent().index()))
                return false;
        }
        return true;
    }
